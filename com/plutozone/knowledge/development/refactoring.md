...
- 뒤엉킨 변경(Divergent Change=단일 책임 원칙 부재 시 발생하는 문제): 분할(단계), 이동(함수), 추출(함수, 클래스) [예] 데이터베이스 종류가 추가될 때마다 함수가 추가됨
...
- 기능 편애(Feature Envy): [예] 내부 코드가 외부 함수 또는 클래스와 상호 작용이 많을 경우
- 데이터 뭉치
- @기본형 집착
- @반복되는 switch문
- @반복문
- 성의 없는 요소(Lazy Element): 전개(함수, 클래스), 통합(로직) [예] 기존 또는 리팩터링 후에 자주 사용하지 않는 로직, 함수 또는 클래스
- 추측성 일반화
- 임시 필드
- 메시지 체인
- 중개자
