...
- summary
	...
	- 예제-2
		...
		- 로컬 1000 포트에 접속할 경우 컨테이너 80 포트로 `p`ort Forwarding(-p 1000:80)
		- 컨테이너 이름은 demoNginx
		- 환경 변수는 ENV=hello
...
$ docker network connect bridge demoApp2									# demoApp2 컨테이너에 기본 bridge 네트워크도 연결(해제: disconnect)
$ docker inspect demoApp2 | grep IP
$ docker rm -f $(docker container ls -a -q)									# 모든 컨테이너 삭제(-f: 강제 중지 후 삭제) or docker ps -aq
$ docker network rm demoNet
...
# 컨테이너 통신 방법 (1) /etc/hosts in bridge 네트워크 (2) --link 옵션 in bridge 네트워크 (3) 자동 이름 검색 서비스(컨테이너명으로 통신 가능) in User definded 네트워크
...
- 유지 운영
	- `이미지`(Image), `태그`(Tag)명에 대문자 사용 불가(`컨테이너명만 가능`)
		- 영문자와 숫자, '-', '_', '.', '/' 만 허용
	- Alpine Image(CoreOS)
		- Alpine Linux는 musl libc와 BusyBox 중심으로 구축된 리눅스 배포판으로 이미지 크기가 9MB에 불과하지만 다른 BusyBox 기반 이미지보다 완전하다는 평가
		```bash
		$ docker run --name demoAlpine -it -d alpine ash
		$ docker attach demoAlpine					# 실행 중인 컨테이너의 연결(기존 프로세스)
		$ docker exec -it demoAlpine /bin/ash		# 실행 중인 컨테이너의 연결(신규 프로세스)
		```
	- 사용하지 않는 이미지 확인
		```bash
